...
const MyQuery = () => {
  ...
  const handleSubjectSelect = (sub, id) => {
    if (selectedTopSubjects?.find((ss) => ss === sub?.subject)) {
      setSelectedTopSubjects((curVal) => curVal.filter((cv) => cv !== sub?.legendsName));
    } else {
                ...
                <div className="my-query-grid-items no-anwer-data">
                  <div className="search-input-alignment-end">
                    <div className="input-relative-search">
                      <input type="search" placeholder="Search for question" id="searchData" onChange={(e) => handleOnChange(e)} value={inputValue} name="searchData" />
                      <div
